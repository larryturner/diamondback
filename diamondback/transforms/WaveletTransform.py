...
class WaveletTransform( object ) :
    ...
    __b__ = dict( Coiflet = { 5 : numpy.array( [ -0.07269889797984, 0.33788487810742, 0.85255707635042, 0.38485416611448,
    ...
    def __init__( self, style : str, order : int ) -> None :
        ...
        if ( style not in WaveletTransform.__b__ ) :
            raise ValueError( f'style = {style} Expected Style in {tuple( WaveletTransform.__b__.keys( ) )}' )
        if ( order not in WaveletTransform.__b__[ style ] ) :
            raise ValueError( f'Order = {order} Expected Order in {tuple( WaveletTransform.__b__[ style ].keys( ) )}' )
        super( ).__init__( )
        b = WaveletTransform.__b__[ style ][ order ]
        n = len( b ) - 1
        b = b / sum( b )
        v = numpy.flip( b, 0 )
        ii = [ 1, 1 ]
